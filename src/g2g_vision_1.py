...
def compute_force(goalx, goaly):
    ...
    dx = np.sin(angles)
    dy = np.cos(angles)
    ranges = np.maximum(ranges - .25, 0.1)
    grad_x = dx/ranges
    grad_y = dy/ranges
...
def compute_force_2(goalx, goaly):
    global ranges, angles
    dx = np.cos(angles)
    dy = np.sin(angles)
    ranges = np.maximum(ranges - .4, 0.01)
    grad_x = dx/ranges
    grad_y = dy/ranges
    mask = ~np.isnan(ranges) & (ranges < 0.5)# & (abs(angles) < .4)
    f_rep_x = -np.sum(grad_x[mask]) / NUM_POINTS
    f_rep_y = -np.sum(grad_y[mask]) / NUM_POINTS
    print('f_rep_x', f_rep_x, 'f_rep_y', f_rep_y)
    f_rep_pub.publish(Vector3(f_rep_x, f_rep_y, 0))
...
def compute_force_1(goalx, goaly):
    global ranges, angles
    dx = np.sin(angles)
    dy = np.cos(angles)
    ranges = np.maximum(ranges - .3, 0.05)
    grad_x = dx/ranges
    grad_y = dy/ranges
    mask = ~np.isnan(ranges) & (ranges < .75)# & (abs(angles) < .4)
    f_rep_x = np.sum((grad_x[mask] - dx/dist_repl_threshold) * np.square(ranges[mask]))
...
def main():
    ...
    # STATE: {0: straight, 1: turn right, 2: turn left}
    #STATE = 0
    #CNT = 0
    Detect = math.pi * 1/6
    ...
    while not rospy.is_shutdown() and not stop:
        ...
        f_x, f_y = compute_force_2(goalx, goaly)
